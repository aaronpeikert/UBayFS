...
#' @param counts a matrix of likelihood counts for the Multinomial distribution
...
posterior = function(state, constraints, block_constraints, weights, counts, covariates = NULL, log = TRUE){								# target function for optimization procedure
  post <-
    admissibility(state, 									# log-admissibility function
                  constraints,
                  sum(weights + colSums(counts)),
                  log = TRUE) +
    block_admissibility(state, 									# log-admissibility function
                        block_constraints,
                        sum(weights + colSums(counts)) / nrow(block_constraints$block_matrix),
                        log = TRUE) +
    ddirichlet(t(state + 0.01), 							# log-dirichlet-density (with small epsilon to avoid errors from 0 probs)
                 alpha = weights,
                 log = TRUE)+
    likelihood(state = counts,
               param = list(t(state + 0.01), covariates),
               log = TRUE)  # likelihood
  if(!log){
    post <- exp(post)
  }
...
getPosterior <- function(state, covariates = NULL, model, log = TRUE){
  return(posterior(
    state = state,
